...
<body>
  ...
  <header class="main_menu">
    ...
    <div class="contents">
    	<div class="btnGroup">
    	<button class="back"><a id="aback" href="community.jsp?type=event&no=<%=no %>&page=<%=pageData%>">목록</a></button>
    	
    	<% if(seb.getId().equals(id)||id.equals("admin")){%>
    		<button class="mod"><a id="aedit" href="eventForm.jsp?type=event&mode=modify&page=<%=pageData %>&no=<%=no%>">수정</a></button>
    		<button class="del"><a id="adelete" href="delete.jsp?type=event&no=<%=no %>">삭제</a></button>
    		<%} %>
  
  <%
  if(id.equals("admin")){
	  %>
    		<button class="del"><a id="ainsert" href="insert.jsp?type=eprogress&no=<%=no %>&page=<%=pageData %>>">마감</a></button>
   		<%
    		}
  %>
    	</div>
